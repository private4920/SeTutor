    ...
    <DashboardLayout>
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-900">
                {selectedFolder ? selectedFolder.name : 'My Files (Root)'}
              </span>
            </div>
            <svg className="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {selectedFolder && (
            <p className="mt-1 text-xs text-gray-500">Path: {selectedFolder.path}</p>
          )}
        </div>

        {/* Upload Error */}
        {uploadError && (
          <div className="mb-6 rounded-lg bg-red-50 p-4 text-sm text-red-600">{uploadError}</div>
        )}
        ...
        {completedUploads > 0 && (
          <div className="mt-6 rounded-lg bg-green-50 p-4">
            ...
            <div className="mt-3">
              <button
                onClick={() => router.push(selectedFolderId ? `/dashboard/files?folderId=${selectedFolderId}` : '/dashboard/files')}
